...
public class Nio2Processor extends AbstractProcessor<Nio2Channel> {
    ...
    public Nio2Processor(AbstractEndpoint<Nio2Channel> endpoint,
            ...
            UpgradeToken upgradeToken, int asyncWriteBufferSize) {
        super(upgradeToken, new Nio2ServletInputStream(wrapper, endpoint),
                new Nio2ServletOutputStream(wrapper, asyncWriteBufferSize,
                        endpoint));

        wrapper.setTimeout(INFINITE_TIMEOUT);
        if (leftoverInput != null) {
            wrapper.getSocket().getBufHandler().getReadBuffer().put(
                    leftoverInput);
        }
    }
}
