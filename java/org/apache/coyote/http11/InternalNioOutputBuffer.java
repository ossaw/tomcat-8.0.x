...
public class InternalNioOutputBuffer extends AbstractOutputBuffer<NioChannel> {
    ...
    public void sendAck() throws IOException {
        if (!committed) {
            ...
            int result = writeToSocket(socket.getBufHandler().getWriteBuffer(),
                    true, true);
            if (result < 0) {
    ...
    private synchronized void addToBB(byte[] buf, int offset, int length)
        ...
        while (!dataLeft && length > 0) {
            int thisTime = transfer(buf, offset, length, socket.getBufHandler()
                    .getWriteBuffer());
        ...
        if (!dataLeft && bufferedWrites.size() > 0) {
            ...
            while (!hasMoreDataToFlush() && bufIter.hasNext()) {
                ...
                while (!hasMoreDataToFlush() && buffer.getBuf()
                        .remaining() > 0) {
                    transfer(buffer.getBuf(), socket.getBufHandler()
                            .getWriteBuffer());
    ...
    protected boolean hasMoreDataToFlush() {
        return (flipped && socket.getBufHandler().getWriteBuffer()
                .remaining() > 0) || (!flipped && socket.getBufHandler()
