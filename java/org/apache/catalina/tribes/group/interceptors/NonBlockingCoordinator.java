 ...
 * it also merges groups automatically when members are discovered that werent
 * part of the
 * </p>
 * <p>
 ...
 * A will send out a VIEW_CONF message, this is the 'confirmed' message that is
 * optional above.
 * <p>
 ...
 *
 * <p>
 * The example above, of course can be simplified with a finite
 * statemachine:<br>
 ...
 * <a href=
 * "http://people.apache.org/~fhanik/tribes/docs/leader-election-initiate-election.jpg">Initiate
 * an election</a><br>
 * <br>
 * <a href=
 * "http://people.apache.org/~fhanik/tribes/docs/leader-election-message-arrives.jpg">Receive
 * an election message</a><br>
 * <br>
 *
 * @version 1.0
 *
...
public class NonBlockingCoordinator extends ChannelInterceptorBase {
    ...
    private static final Log log = LogFactory.getLog(
            NonBlockingCoordinator.class);
    protected static final StringManager sm = StringManager.getManager(
            NonBlockingCoordinator.class.getPackage().getName());

    /**
     * header for a coordination message
     */
    protected static final byte[] COORD_HEADER = new byte[] { -86, 38, -34, -29,
     ...
     * Alive message
     */
    protected static final byte[] COORD_ALIVE = new byte[] { 79, -121, -25, -15,
    ...
    public void startElection(boolean force) throws ChannelException {
        synchronized (electionMutex) {
            ...
            if (suggestedviewId != null) {
                ...
                if (view != null && Arrays.diff(view, suggestedView,
                        local).length == 0 && Arrays.diff(suggestedView, view,
                                local).length == 0) {
    ...
    private CoordinationMessage createElectionMsg(Member local, Member[] others,
            Member leader) {
        ...
        while (!sent && current >= 0) {
            try {
                ...
                log.warn(sm.getString(
                        "nonBlockingCoordinator.electionMessage.sendfailed", msg
                                .getMembers()[current]));
                current = Arrays.nextIndex(msg.getMembers()[current], msg
                        .getMembers());
                if (current == next)
                    throw x;
            }
    ...
    protected boolean alive(Member mbr) {
        return TcpFailureDetector.memberAlive(mbr, COORD_ALIVE, false, false,
    ...
    protected void processCoordMessage(CoordinationMessage msg)
    ...
    protected void handleViewConf(CoordinationMessage msg, Membership merged)
        ...
        if (suggestedView != null && AbsoluteOrder.comp.compare(suggestedView
                .getMembers()[0], merged.getMembers()[0]) < 0) {
            suggestedView = null;
            suggestedviewId = null;
        }
        ...
        if (suggestedviewId == null && hasHigherPriority(merged.getMembers(),
    ...
    public void start(int svc) throws ChannelException {
        ...
        started = true;
        if (view == null)
            view = new Membership(super.getLocalMember(true),
    ...
    public void messageReceived(ChannelMessage msg) {
        if (Arrays.contains(msg.getMessage().getBytesDirect(), 0, COORD_ALIVE,
            ...
            try {
                CoordinationMessage cmsg = new CoordinationMessage(msg
                        .getMessage());
                ...
                log.error(sm.getString(
                        "nonBlockingCoordinator.processCoordinationMessage.failed"),
                        x);
            }
        } else {
    ...
    public void memberAdded(Member member, boolean elect) {
        ...
        try {
            fireInterceptorEvent(new CoordinationEvent(
                    CoordinationEvent.EVT_MBR_ADD, this, "Member add(" + member
                            .getName() + ")"));
            if (started && elect)
                startElection(false);
        } catch (ChannelException x) {
            log.error(sm.getString("nonBlockingCoordinator.memberAdded.failed"),
                    x);
        }
    }
    ...
    public boolean isHighest() {
        ...
        if (membership.getMembers().length == 0)
            ...
            return AbsoluteOrder.comp.compare(local, membership
                    .getMembers()[0]) <= 0;
    }
    ...
    public void heartbeat() {
        try {
            Member local = getLocalMember(false);
            if (view != null && (Arrays.diff(view, membership,
                    local).length != 0 || Arrays.diff(membership, view,
                            local).length != 0)) {
                if (isHighest()) {
                    fireInterceptorEvent(new CoordinationEvent(
                            CoordinationEvent.EVT_START_ELECT, this, sm
                                    .getString(
                                            "nonBlockingCoordinator.heartbeat.inconsistency")));
                    startElection(true);
                }
            }
        } catch (Exception x) {
    ...
    public static class CoordinationMessage {
        ...
        public CoordinationMessage(Member leader, Member source, Member[] view,
                UniqueId id, byte[] type) {
    ...
    public static class CoordinationEvent implements InterceptorEvent {
        ...
        public String toString() {
            ...
            buf.append(coord != null ? coord.getName() : "").append(
                    "\n\tView:");
            buf.append(Arrays.toNameString(view != null ? view.getMembers()
                    : null)).append("\n\tSuggested View:");
            buf.append(Arrays.toNameString(suggestedView != null ? suggestedView
