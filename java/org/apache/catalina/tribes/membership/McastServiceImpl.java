...
public class McastServiceImpl {
     ...
     */
    protected final ExecutorService executor = ExecutorFactory.newThreadPool(0,
    ...
    protected void setupSocket() throws IOException {
        if (mcastBindAddress != null) {
            try {
                log.info(sm.getString("mcastServiceImpl.bind", address, Integer
                        .toString(port)));
        ...
        if (log.isInfoEnabled()) {
            log.info(sm.getString("mcastServiceImpl.setSoTimeout", Integer
                    .toString(mcastSoTimeout)));
        }
        socket.setSoTimeout(mcastSoTimeout);

        if (mcastTTL >= 0) {
            if (log.isInfoEnabled())
                log.info(sm.getString("mcastServiceImpl.setTTL", Integer
                        .toString(mcastTTL)));
        ...
        if ((level & Channel.MBR_RX_SEQ) == Channel.MBR_RX_SEQ) {
            if (receiver != null)
                throw new IllegalStateException(sm.getString(
                        "mcastServiceImpl.receive.running"));
            try {
                if (sender == null)
    ...
    private void waitForMembers(int level) {
        ...
        if (log.isInfoEnabled())
            log.info(sm.getString("mcastServiceImpl.waitForMembers.start", Long
                    .toString(memberwait), Integer.toString(level)));
        try {
    ...
    public synchronized boolean stop(int level) throws IOException {
        ...
        if (!valid) {
            throw new IllegalArgumentException(sm.getString(
                    "mcastServiceImpl.invalid.stopLevel"));
        }
        startLevel = (startLevel & (~level));
    ...
    public void receive() throws IOException {
        ...
        try {
            ...
            if (receivePacket.getLength() > MAX_PACKET_SIZE) {
                ...
                System.arraycopy(receivePacket.getData(), receivePacket
                        .getOffset(), data, 0, data.length);
                ...
                public void run() {
                    ...
                    try {
                        ...
                        for (int i = 0; i < data.length; i++) {
                            try {
                                if (data[i] != null && !member.equals(data[i]
                                        .getAddress())) {
                                ...
                                log.error(sm.getString(
                                        "mcastServiceImpl.unableReceive.broadcastMessage"),
                                        t);
                            }
                        }
                    } finally {
        ...
        synchronized (expiredMutex) {
            ...
            for (int i = 0; i < expired.length; i++) {
                ...
                try {
                    ...
                    log.error(sm.getString(
                            "mcastServiceImpl.memberDisappeared.failed"), x);
                }
            }
        }
    }
                ...
                try {
                    ...
                    if (errorCounter == 0 && doRunReceiver)
                        log.warn(sm.getString(
                                "mcastServiceImpl.error.receiving"), x);
    ...
    protected static class RecoveryThread extends Thread {
        ...
        public boolean startService() {
            try {
                ...
                log.warn(sm.getString("mcastServiceImpl.recovery.startFailed"),
                        x);
                return false;
            }
        }
                    ...
                    if (stopService() & startService()) {
                        ...
                        if (log.isInfoEnabled())
                            log.info(sm.getString(
                                    "mcastServiceImpl.recovery.successful"));
                    }
                    try {
                        if (!success) {
                            if (log.isInfoEnabled())
                                log.info(sm.getString(
                                        "mcastServiceImpl.recovery.failed",
                                        Integer.toString(++attempt), Long
                                                .toString(
