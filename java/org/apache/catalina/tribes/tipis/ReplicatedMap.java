 ...
 * <code>ReplicatedMapEntry</code> interface
 * each time the object gets replicated the entire object gets serialized, hence
 * a call to <code>replicate(true)</code>
 * will replicate all objects in this map that are using this node as primary.
 *
 * <br>
 * <br>
 * <b>REMEMBER TO CALL <code>breakdown()</code> or <code>finalize()</code>
 * when you are done with the map to avoid memory leaks.</b><br>
 * <br>
...
public class ReplicatedMap<K, V> extends AbstractReplicatedMap<K, V> {
    ...
    protected Member[] publishEntryInfo(Object key, Object value)
        ...
        try {
            ...
            for (FaultyMember faultyMember : faultyMembers) {
                if (!(faultyMember
                        .getCause() instanceof RemoteProcessException)) {
                    faulty.add(faultyMember.getMember());
                }
            }
            Member[] realFaultyMembers = faulty.toArray(new Member[faulty
                    .size()]);
                ...
                if (backup.length == 0) {
                    ...
                    if (log.isWarnEnabled()) {
                        log.warn(sm.getString(
                                "replicatedMap.unableReplicate.completely", key,
                                Arrays.toString(backup), Arrays.toString(
                                        realFaultyMembers)), e);
                    }
                }
            }
    ...
    public void memberDisappeared(Member member) {
        ...
        while (i.hasNext()) {
            ...
            if (entry.isPrimary()) {
                try {
                    ...
                    log.error(sm.getString("replicatedMap.unable.relocate",
                            entry.getKey()), x);
                }
            ...
            if (entry.getPrimary() == null && entry.isCopy() && entry
                    .getBackupNodes() != null && entry
                            .getBackupNodes().length > 0 && entry
        ...
        if (log.isInfoEnabled())
            log.info(sm.getString("replicatedMap.relocate.complete", Long
                    .toString(complete)));
    }
    ...
    public void mapMemberAdded(Member member) {
        ...
        synchronized (mapMembers) {
            if (!mapMembers.containsKey(member)) {
                mapMembers.put(member, Long.valueOf(System
                        .currentTimeMillis()));
                memberAdded = true;
            }
        }
        if (memberAdded) {
            synchronized (stateMutex) {
                ...
                while (i.hasNext()) {
                    ...
                    if (entry == null)
                        continue;
                    if (entry.isPrimary() && !inSet(member, entry
                            .getBackupNodes())) {
                        entry.setBackupNodes(backup);
                    }
                }
