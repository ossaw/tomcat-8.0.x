...
public abstract class AbstractReplicatedMap<K, V> implements Map<K, V>,
    ...
    protected static final StringManager sm = StringManager.getManager(
            AbstractReplicatedMap.class.getPackage().getName());
        ...
        try {
            ...
            if (terminate) {
                ...
                throw new RuntimeException(sm.getString(
                        "abstractReplicatedMap.unableStart"), x);
            }
        }
    ...
    protected void ping(long timeout) throws ChannelException {
        ...
        MapMessage msg = new MapMessage(this.mapContextName,
                MapMessage.MSG_PING, false, null, null, null, channel
                        ...
                        if (state.isAvailable()) {
                            ...
                            if (log.isInfoEnabled())
                                log.info(sm.getString(
                                        "abstractReplicatedMap.mapMember.unavailable",
                                        resp[i].getSource()));
                        }
                    } catch (ClassNotFoundException | IOException e) {
                        log.error(sm.getString(
                                "abstractReplicatedMap.unable.deserialize.MapMessage"),
                                e);
                    }
                }
        ...
        synchronized (mapMembers) {
            Member[] members = mapMembers.keySet().toArray(new Member[mapMembers
                    .size()]);
            long now = System.currentTimeMillis();
            for (Member member : members) {
                long access = mapMembers.get(member).longValue();
                if ((now - access) > timeout) {
                    log.warn(sm.getString("abstractReplicatedMap.ping.timeout",
                            member, mapname));
    ...
    protected void broadcast(int msgtype, boolean rpc) throws ChannelException {
        ...
        if (rpc) {
            ...
            if (resp.length > 0) {
                ...
                log.warn(sm.getString(
                        "abstractReplicatedMap.broadcast.noReplies"));
            }
        } else {
     ...
     *                 if set to false, only objects that implement
     *                 ReplicatedMapEntry and the isDirty() returns true will
     *                 be replicated
     */
    public void replicate(Object key, boolean complete) {
        ...
        if (!entry.isSerializable())
            return;
        if (entry.isPrimary() && entry.getBackupNodes() != null && entry
            ...
            if (rentry != null && rentry.isDiffable() && (isDirty
                ...
                try {
                    ...
                    msg = new MapMessage(mapContextName,
                            getReplicateMessageType(), true,
                            (Serializable) entry.getKey(), null, rentry
            ...
            if (msg == null) {
                ...
                msg = new MapMessage(mapContextName, MapMessage.MSG_ACCESS,
                        false, (Serializable) entry.getKey(), null, null, entry
                                .getPrimary(), entry.getBackupNodes());
            }
            try {
                if (channel != null && entry.getBackupNodes() != null && entry
                        .getBackupNodes().length > 0) {
                    if (rentry != null)
                        rentry.setLastTimeReplicated(System
                                .currentTimeMillis());
    ...
    public Serializable replyRequest(Serializable msg, final Member sender) {
        ...
        if (mapmsg.getMsgType() == MapMessage.MSG_STATE || mapmsg
                .getMsgType() == MapMessage.MSG_STATE_COPY) {
            synchronized (stateMutex) { //make sure we dont do two things at the same time
                ...
                while (i.hasNext()) {
                    ...
                    if (entry != null && entry.isSerializable()) {
                        boolean copy = (mapmsg
                                .getMsgType() == MapMessage.MSG_STATE_COPY);
                        MapMessage me = new MapMessage(mapContextName, copy
                                ? MapMessage.MSG_COPY
                                : MapMessage.MSG_PROXY, false,
                                (Serializable) entry.getKey(), copy
                                        ? (Serializable) entry.getValue()
                                        : null, null, entry.getPrimary(), entry
                                                .getBackupNodes());
                        list.add(me);
                    }
                }
    ...
    public void leftOver(Serializable msg, Member sender) {
        ...
        try {
            ...
            } else if (mapmsg.getMsgType() == MapMessage.MSG_INIT || mapmsg
                    .getMsgType() == MapMessage.MSG_PING) {
                memberAlive(mapmsg.getPrimary());
            } else {
                // other messages are ignored.
                if (log.isInfoEnabled())
                    log.info(sm.getString(
                            "abstractReplicatedMap.leftOver.ignored", mapmsg
                                    .getTypeDesc()));
            }
        } catch (IOException x) {
            log.error(sm.getString(
    ...
    public void messageReceived(Serializable msg, Member sender) {
        ...
        if (mapmsg.getMsgType() == MapMessage.MSG_PROXY) {
            MapEntry<K, V> entry = innerMap.get(mapmsg.getKey());
            if (entry == null) {
                entry = new MapEntry<>((K) mapmsg.getKey(), (V) mapmsg
                        .getValue());
        ...
        if (mapmsg.getMsgType() == MapMessage.MSG_BACKUP || mapmsg
                .getMsgType() == MapMessage.MSG_COPY) {
            MapEntry<K, V> entry = innerMap.get(mapmsg.getKey());
            if (entry == null) {
                entry = new MapEntry<>((K) mapmsg.getKey(), (V) mapmsg
                        .getValue());
                ...
                entry.setBackupNodes(mapmsg.getBackupNodes());
                entry.setPrimary(mapmsg.getPrimary());
                if (mapmsg.getValue() != null && mapmsg
                        .getValue() instanceof ReplicatedMapEntry) {
                    ((ReplicatedMapEntry) mapmsg.getValue()).setOwner(
                            getMapOwner());
                }
            } else {
                entry.setBackup(mapmsg.getMsgType() == MapMessage.MSG_BACKUP);
                ...
                if (entry.getValue() instanceof ReplicatedMapEntry) {
                    ...
                    if (mapmsg.isDiff()) {
                        ...
                        try {
                            ...
                            log.error(sm.getString(
                                    "abstractReplicatedMap.unableApply.diff",
                                    entry.getKey()), x);
                        } finally {
                            diff.unlock();
                        }
                        ...
                        if (mapmsg.getValue() != null) {
                            ...
                            ((ReplicatedMapEntry) entry.getValue()).setOwner(
                                    getMapOwner());
                        }
                    } //end if
    ...
    public void mapMemberAdded(Member member) {
        ...
        if (mapMember == null) {
            log.warn(sm.getString(
                    "abstractReplicatedMap.mapMemberAdded.nullMember", member));
            return;
        }
        synchronized (mapMembers) {
            if (!mapMembers.containsKey(mapMember)) {
                if (log.isInfoEnabled())
                    log.info(sm.getString(
                            "abstractReplicatedMap.mapMemberAdded.added",
                            mapMember));
                mapMembers.put(mapMember, Long.valueOf(System
                        .currentTimeMillis()));
                mapMembers.put(mapMember, Long.valueOf(System
                        .currentTimeMillis()));
                memberAdded = true;
                ...
                while (i.hasNext()) {
                    ...
                    if (entry.isPrimary() && (entry.getBackupNodes() == null
                        ...
                        try {
                            ...
                            log.error(sm.getString(
                                    "abstractReplicatedMap.unableSelect.backup"),
                                    x);
                        } //catch
                    } //end if
                } //while
        ...
        while (i.hasNext()) {
            ...
            if (entry.isPrimary() && inSet(member, entry.getBackupNodes())) {
                ...
                try {
                    ...
                    log.error(sm.getString(
                            "abstractReplicatedMap.unable.relocate", entry
                                    .getKey()), x);
                }
            ...
            if (entry.isProxy() && entry.getPrimary() == null && entry
                    .getBackupNodes() != null && entry
                            .getBackupNodes().length == 1 && entry
                                    .getBackupNodes()[0].equals(member)) {
                //remove proxies that have no backup nor primaries
                if (log.isDebugEnabled())
                    log.debug("[3] Removing orphaned proxy");
                i.remove();
            } else if (entry.getPrimary() == null && entry.isBackup() && entry
                    .getBackupNodes() != null && entry
                            .getBackupNodes().length == 1 && entry
                                    .getBackupNodes()[0].equals(channel
                ...
                try {
                    ...
                    entry.setBackupNodes(backup);
                    if (mapOwner != null)
                        mapOwner.objectMadePrimary(entry.getKey(), entry
                                .getValue());
        ...
        if (!entry.isPrimary()) {
            ...
            try {
                ...
                if (entry.isBackup()) {
                    ...
                    Response[] resp = getRpcChannel().send(entry
                            .getBackupNodes(), msg, RpcChannel.FIRST_REPLY,
                            Channel.SEND_OPTIONS_DEFAULT, getRpcTimeout());
                    if (resp == null || resp.length == 0 || resp[0]
                            .getMessage() == null) {
                        //no responses
                        log.warn(sm.getString(
                                "abstractReplicatedMap.unable.retrieve", key));
                        return null;
                    }
                    msg = (MapMessage) resp[0].getMessage();
                    msg.deserialize(getExternalLoaders());
                    ...
                    msg = new MapMessage(getMapContextName(),
                            ...
                            (Serializable) entry.getKey(), null, null, channel
                                    .getLocalMember(false), backup);
                    ...
                    if (backup.length > 0) {
                        msg = new MapMessage(getMapContextName(),
                                MapMessage.MSG_NOTIFY_MAPMEMBER, false,
                                (Serializable) key, null, null, channel
                                        .getLocalMember(false), backup);
                        getChannel().send(backup, msg, getChannelSendOptions());
                    }
                }
    ...
    protected void printMap(String header) {
        try {
            System.out.println("\nDEBUG MAP:" + header);
            System.out.println("Map[" + new String(mapContextName,
                    StandardCharsets.ISO_8859_1) + ", Map Size:" + innerMap
                            .size());
    ...
    public boolean containsValue(Object value) {
        ...
        while (i.hasNext()) {
            Map.Entry<K, MapEntry<K, V>> e = i.next();
            MapEntry<K, V> entry = innerMap.get(e.getKey());
            if (entry != null && entry.isActive() && value.equals(entry
                    .getValue()))
                return true;
        }
        return false;
    ...
    public Object clone() {
        throw new UnsupportedOperationException(sm.getString(
                "abstractReplicatedMap.unsupport.operation"));
    }
    ...
    public Set<Map.Entry<K, V>> entrySet() {
        LinkedHashSet<Map.Entry<K, V>> set = new LinkedHashSet<>(innerMap
                .size());
        Iterator<Map.Entry<K, MapEntry<K, V>>> i = innerMap.entrySet()
                .iterator();
        while (i.hasNext()) {
    ...
    public int size() {
        ...
        while (it != null && it.hasNext()) {
            ...
            if (e != null) {
                MapEntry<K, V> entry = innerMap.get(e.getKey());
                if (entry != null && entry.isActive() && entry
                        .getValue() != null)
                    counter++;
            }
        }
    ...
    public static class MapMessage implements Serializable {
        ...
        public Serializable getKey() {
            try {
                ...
                throw new RuntimeException(sm.getString(
                        "mapMessage.deserialize.error.key"), x);
            }
        }
        ...
        public Serializable getValue() {
            try {
                ...
                throw new RuntimeException(sm.getString(
                        "mapMessage.deserialize.error.value"), x);
            }
        }
        ...
        public Object clone() {
            MapMessage msg = new MapMessage(this.mapId, this.msgtype, this.diff,
                    this.key, this.value, this.diffvalue, this.primary,
                    this.nodes);
    ...
    private static enum State {
        NEW(false), INITIALIZED(true), DESTROYED(false);
