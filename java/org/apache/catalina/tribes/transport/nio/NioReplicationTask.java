...
public class NioReplicationTask extends AbstractRxTask {
    ...
    protected static final StringManager sm = StringManager.getManager(
            NioReplicationTask.class.getPackage().getName());
            ...
            if (e instanceof CancelledKeyException) {
                ...
                if (log.isDebugEnabled())
                    ...
                    log.warn(sm.getString(
                            "nioReplicationTask.unable.drainChannel.ioe", e
                                    .getMessage()));
    ...
    protected void drainChannel(final SelectionKey key, ObjectReader reader)
        ...
        if (channel instanceof SocketChannel) {
            ...
            buffer.flip(); // make buffer readable
            if (buffer.hasArray())
                reader.append(buffer.array(), 0, buffer.limit() - buffer
                        .position(), false);
            else
        ...
        for (int i = 0; i < msgs.length; i++) {
            ...
            try {
                if (Logs.MESSAGES.isTraceEnabled()) {
                    try {
                        Logs.MESSAGES.trace(
                                "NioReplicationThread - Received msg:"
                                        + new UniqueId(msgs[i].getUniqueId())
                                        + " at " + new java.sql.Timestamp(System
                                                .currentTimeMillis()));
                    } catch (Throwable t) {
                    }
                }
    ...
    protected void sendAck(SelectionKey key, WritableByteChannel channel,
        ...
        try {
            ...
            if (log.isTraceEnabled()) {
                log.trace("ACK sent to " + ((channel instanceof SocketChannel)
                        ? ((SocketChannel) channel).socket().getInetAddress()
                        : ((DatagramChannel) channel).socket()
                                .getInetAddress()));
            }
        } catch (java.io.IOException x) {
            log.warn(sm.getString("nioReplicationTask.unable.ack", x
                    .getMessage()));
        }
    }
