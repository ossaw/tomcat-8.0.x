...
public class NioReceiver extends ReceiverBase implements Runnable {
    ...
    protected static final StringManager sm = StringManager.getManager(
            NioReceiver.class.getPackage().getName());
                        ...
                        if (delta > getTimeout() && (!ka.isAccessed())) {
                            if (log.isWarnEnabled())
                                log.warn(sm.getString(
                                        "nioReceiver.threadsExhausted", Integer
                                                .valueOf(getTimeout()), Boolean
                                                        .valueOf(ka
                                                                .isCancelled()),
                                        key, new java.sql.Timestamp(ka
                                                .getLastAccess())));
    ...
    protected void registerChannel(Selector selector, SelectableChannel channel,
            int ops, Object attach) throws Exception {
        if (channel == null)
            return; // could happen
