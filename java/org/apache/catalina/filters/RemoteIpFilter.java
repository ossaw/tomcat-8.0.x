 ...
 * href=
 * "http://httpd.apache.org/docs/trunk/mod/mod_remoteip.html">mod_remoteip</a>,
 * this servlet filter replaces the apparent client remote
 * IP address and hostname for the request with the IP address list presented by
 * a proxy or a load balancer via a request headers (e.g.
 * "X-Forwarded-For").
 * </p>
 * <p>
 * Another feature of this servlet filter is to replace the apparent scheme
 * (http/https) and server port with the scheme presented by a
 ...
 * href=
 * "http://apr.apache.org/docs/apr/1.3/group__apr__network__io.html#gb74d21b8898b7c40bf7fd07ad3eb993d">apr_ipsubnet_test</a>,
 * we rely on
 * regular expressions.
 * </p>
 * <hr>
 ...
 * <code>x-forwarded-by</code>. <code>x-forwarded-by</code> is null because all
...
public class RemoteIpFilter implements Filter {
    public static class XForwardedRequest extends HttpServletRequestWrapper {
        ...
        static final ThreadLocal<SimpleDateFormat[]> threadLocalDateFormats = new ThreadLocal<SimpleDateFormat[]>() {
            ...
            protected SimpleDateFormat[] initialValue() {
                return new SimpleDateFormat[] { new SimpleDateFormat(
                        "EEE, dd MMM yyyy HH:mm:ss zzz", Locale.US),
                        new SimpleDateFormat("EEEEEE, dd-MMM-yy HH:mm:ss zzz",
                                Locale.US), new SimpleDateFormat(
        ...
        public XForwardedRequest(HttpServletRequest request) {
            ...
            headers = new HashMap<>();
            for (Enumeration<String> headerNames = request
                    .getHeaderNames(); headerNames.hasMoreElements();) {
        ...
        public String getHeader(String name) {
            ...
            if (header == null || header.getValue() == null || header.getValue()
                    .isEmpty()) {
                return null;
            }
        ...
        public StringBuffer getRequestURL() {
            ...
            if ((scheme.equals("http") && (port != 80)) || (scheme.equals(
                    "https") && (port != 443))) {
                url.append(':');
                url.append(port);
            }
    ...
    protected static String[] commaDelimitedListToStringArray(
        ...
        return (commaDelimitedStrings == null || commaDelimitedStrings
                .length() == 0) ? new String[0]
                        : commaSeparatedValuesPattern.split(
     ...
     */
    protected static String listToCommaDelimitedString(
    ...
    public void doFilter(HttpServletRequest request,
        ...
        if (internalProxies != null && internalProxies.matcher(request
            ...
            String[] remoteIpHeaderValue = commaDelimitedListToStringArray(
                    concatRemoteIpHeaderValue.toString());
            int idx;
            ...
            for (idx = remoteIpHeaderValue.length - 1; idx >= 0; idx--) {
                ...
                } else if (trustedProxies != null && trustedProxies.matcher(
                        currentRemoteIp).matches()) {
            ...
            if (remoteIp != null) {
                ...
                if (proxiesHeaderValue.size() == 0) {
                    ...
                    String commaDelimitedListOfProxies = listToCommaDelimitedString(
                            proxiesHeaderValue);
                ...
                if (newRemoteIpHeaderValue.size() == 0) {
                    ...
                    String commaDelimitedRemoteIpHeaderValue = listToCommaDelimitedString(
            ...
            if (protocolHeader != null) {
                ...
                } else if (protocolHeaderHttpsValue.equalsIgnoreCase(
                        protocolHeaderValue)) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Incoming request " + request.getRequestURI()
                        ...
                        + "' will be seen as newRemoteAddr='" + xRequest
                                .getRemoteAddr() + "', newRemoteHost='"
                        + xRequest.getRemoteHost() + "', newScheme='" + xRequest
                                .getScheme() + "', newSecure='" + xRequest
                                        .isSecure() + "', new[" + remoteIpHeader
                        + "]='" + xRequest.getHeader(remoteIpHeader) + "', new["
                        + proxiesHeader + "]='" + xRequest.getHeader(
                                proxiesHeader) + "'");
            }
            if (requestAttributesEnabled) {
                request.setAttribute(AccessLog.REMOTE_ADDR_ATTRIBUTE, xRequest
                        .getRemoteAddr());
                request.setAttribute(Globals.REMOTE_ADDR_ATTRIBUTE, xRequest
                        .getRemoteAddr());
                request.setAttribute(AccessLog.REMOTE_HOST_ATTRIBUTE, xRequest
                        .getRemoteHost());
                request.setAttribute(AccessLog.PROTOCOL_ATTRIBUTE, xRequest
                        .getProtocol());
                request.setAttribute(AccessLog.SERVER_PORT_ATTRIBUTE, Integer
                        .valueOf(xRequest.getServerPort()));
            }
            chain.doFilter(xRequest, response);
        } else {
            if (log.isDebugEnabled()) {
                log.debug("Skip RemoteIpFilter for request " + request
                        .getRequestURI() + " with originalRemoteAddr '"
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        if (filterConfig.getInitParameter(
                PROTOCOL_HEADER_HTTPS_VALUE_PARAMETER) != null) {
            setProtocolHeaderHttpsValue(filterConfig.getInitParameter(
        ...
        if (filterConfig.getInitParameter(
                CHANGE_LOCAL_PORT_PARAMETER) != null) {
        ...
        if (filterConfig.getInitParameter(
                HTTPS_SERVER_PORT_PARAMETER) != null) {
            try {
