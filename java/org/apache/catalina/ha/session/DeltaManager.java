...
public class DeltaManager extends ClusterManagerBase {
    ...
    /**
     * @param sendAllSessionsWaitTime The sendAllSessionsWaitTime to set at
     *                                msec.
     */
    ...
    protected void sendChangeSessionId(String newSessionID,
            String orgSessionID) {
        if (cluster.getMembers().length > 0) {
            try {
                // serialize sessionID
                ...
                SessionMessage msg = new SessionMessageImpl(getName(),
                        ...
                        orgSessionID, orgSessionID + "-" + System
                                .currentTimeMillis());
     ...
     *                                   if a serialized class cannot be found
     *                                   during the reload
                ...
                if (findSession(session.getIdInternal()) == null) {
                    ...
                    if (log.isWarnEnabled()) {
                        log.warn(sm.getString(
                                "deltaManager.loading.existing.session", session
                                        .getIdInternal()));
                    }
                }
                add(session);
    ...
    protected Member findSessionMasterMember() {
        ...
        if (mbr != null && log.isDebugEnabled()) {
            log.debug(sm.getString("deltaManager.foundMasterMember", getName(),
                    mbr));
        }
        return mbr;
    }
        ...
        if (isTimeout) {
            ...
            if (log.isWarnEnabled())
                log.warn(sm.getString("deltaManager.noContextManager",
                        getName(), new Date(beforeSendTime), Long.valueOf(reqNow
                                - beforeSendTime)));
        } else {
            if (log.isInfoEnabled())
    ...
    public ClusterMessage requestCompleted(String sessionId, boolean expires) {
        ...
        if (!expires && (msg == null)) {
            long replDelta = System.currentTimeMillis() - session
                    .getLastTimeReplicated();
            if (session.getMaxInactiveInterval() >= 0 && replDelta > (session
    ...
    protected void sendSessions(Member sender, Session[] currentSessions,
        ...
        if (log.isDebugEnabled()) {
            log.debug(sm.getString("deltaManager.receiveMessage.unloadingAfter",
                    getName()));
        }
        SessionMessage newmsg = new SessionMessageImpl(name,
