...
public class RewriteRule {
    ...
    public CharSequence evaluate(CharSequence url, Resolver resolver) {
        ...
        while (!done) {
            if (pos < conditions.length) {
                rewrite = conditions[pos].evaluate(matcher, lastMatcher,
                        resolver);
                if (rewrite) {
                    ...
                    while (pos < conditions.length && conditions[pos]
                            .isOrnext()) {
                        pos++;
                    }
        ...
        if (rewrite) {
            if (isEnv()) {
                for (int i = 0; i < envSubstitution.size(); i++) {
                    envResult.get(i).set(envSubstitution.get(i).evaluate(
                            matcher, lastMatcher, resolver));
                }
            }
            if (isCookie()) {
                cookieResult.set(cookieSubstitution.evaluate(matcher,
     ...
     * string
     * is a valid URI (e.g., typically starting with http://hostname) which can
     * be
