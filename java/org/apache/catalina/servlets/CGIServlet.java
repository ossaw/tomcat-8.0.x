...
public final class CGIServlet extends HttpServlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        boolean passShellEnvironment = Boolean.parseBoolean(getServletConfig()
                .getInitParameter("passShellEnvironment"));
        ...
        if (getServletConfig().getInitParameter("envHttpHeaders") != null) {
            envHttpHeadersPattern = Pattern.compile(getServletConfig()
                    .getInitParameter("envHttpHeaders"));
        }
    }
    ...
    private void printServletEnvironment(HttpServletRequest req)
        ...
        while (attrs.hasMoreElements()) {
            ...
            log.trace("Request Attribute: " + attr + ": [ " + req.getAttribute(
                    attr) + "]");
        }
        ...
        if (cookies != null) {
            for (Cookie cookie : cookies) {
                log.trace("Cookie: " + cookie.getName() + ": [" + cookie
                        .getValue() + "]");
            }
        }
        ...
        log.trace("Requested Session ID: [" + req.getRequestedSessionId()
                + "]");
        log.trace("Requested Session ID From Cookie: [" + req
                .isRequestedSessionIdFromCookie() + "]");
        log.trace("Requested Session ID From URL: [" + req
                .isRequestedSessionIdFromURL() + "]");
        log.trace("Requested Session ID Valid: [" + req
        ...
        if (session != null) {
            ...
            log.trace("Creation Time: [" + new Date(session.getCreationTime())
                    + "]");
            log.trace("Last Accessed Time: [" + new Date(session
                    .getLastAccessedTime()) + "]");
            log.trace("Max Inactive Interval: [" + session
            ...
            while (attrs.hasMoreElements()) {
                String attr = attrs.nextElement();
                log.trace("Session Attribute: " + attr + ": [" + session
                        .getAttribute(attr) + "]");
            }
        }
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse res)
        ...
        if (cgiEnv.isValid()) {
            CGIRunner cgi = new CGIRunner(cgiEnv.getCommand(), cgiEnv
                    .getEnvironment(), cgiEnv.getWorkingDirectory(), cgiEnv
                            .getParameters());
        ...
        if (log.isTraceEnabled()) {
            String[] cgiEnvLines = cgiEnv.toString().split(System
                    .lineSeparator());
    ...
    protected class CGIEnvironment {
        ...
        protected CGIEnvironment(HttpServletRequest req, ServletContext context)
            ...
            if (this.valid) {
                workingDirectory = new File(command.substring(0, command
            ...
            if (req.getMethod().equals("GET") || req.getMethod().equals("POST")
                ...
                if (qs != null && qs.indexOf('=') == -1) {
                    ...
                    while (qsTokens.hasMoreTokens()) {
                        cmdLineParameters.add(URLDecoder.decode(qsTokens
                                .nextToken(), parameterEncoding));
                    }
                }
            }
        }
        ...
        protected String[] findCGI(String pathInfo, String webAppRootDir,
            ...
            if (webAppRootDir != null && webAppRootDir.lastIndexOf(
                    File.separator) == (webAppRootDir.length() - 1)) {
                //strip the trailing "/" from the webAppRootDir
                webAppRootDir = webAppRootDir.substring(0, (webAppRootDir
                        .length() - 1));
            }
        ...
        protected boolean setCGIEnvironment(HttpServletRequest req)
            ...
            Integer iPort = (port == 0 ? Integer.valueOf(-1)
             ...
             *
             */
            if (pathInfo == null || (pathInfo.substring(sCGIFullName.length())
                    .length() <= 0)) {
                sPathInfoCGI = "";
            } else {
            ...
            while (headers.hasMoreElements()) {
                ...
                if (envHttpHeadersPattern.matcher(header).matches()) {
                    envp.put("HTTP_" + header.replace('-', '_'), req.getHeader(
                            header));
                }
            }
        ...
        protected void expandCGIScript() {
            ...
            if (!dir.mkdirs() && !dir.isDirectory()) {
                log.warn(sm.getString("cgiServlet.expandCreateDirFail", dir
                        .getAbsolutePath()));
                return;
            }
            ...
            try {
                ...
                if (f.exists()) {
                    if (!f.delete()) {
                        log.warn(sm.getString("cgiServlet.expandDeleteFail", f
                                .getAbsolutePath()));
                    }
                }
            }
        ...
        public String toString() {
            ...
            if (isValid()) {
                ...
                for (Entry<String, String> entry : env.entrySet()) {
                    ...
                    sb.append(": [");
                    sb.append(blanksToString(entry.getValue(),
                ...
                sb.append(System.lineSeparator());
                sb.append(
                        "Check the HttpServletRequest pathInfo property to see if it is what ");
                sb.append(System.lineSeparator());
                sb.append(
                        "you meant it to be. You must specify an existant and executable file ");
                sb.append(System.lineSeparator());
    ...
    protected class CGIRunner {
        ...
        protected void run() throws IOException {
            ...
            if ((command.indexOf(File.separator + "." + File.separator) >= 0)
                    || (command.indexOf(File.separator + "..") >= 0) || (command
                            .indexOf(".." + File.separator) >= 0)) {
                throw new IOException(this.getClass().getName()
            ...
            try {
                ...
                proc = rt.exec(cmdAndArgs.toArray(new String[cmdAndArgs
                        .size()]), hashToStringArray(env), wd);
                    ...
                    try {
                        ...
                        while (((line = cgiHeaderReader.readLine()) != null)
                            ...
                            if (line.startsWith("HTTP")) {
                                ...
                                log.info(sm.getString("cgiServlet.runBadHeader",
                                        line));
                            }
                        }
                        ...
                        try {
                            while (!skipBody && (bufRead = cgiOutput.read(
                                    bBuf)) != -1) {
                                if (log.isTraceEnabled()) {
                ...
                if (cgiHeaderReader != null) {
                    try {
                        ...
                        log.warn(sm.getString("cgiServlet.runHeaderReaderFail"),
                                ioe);
                    }
                }
                ...
                if (cgiOutput != null) {
                    try {
                        ...
                        log.warn(sm.getString("cgiServlet.runOutputStreamFail"),
                                ioe);
                    }
                }
        ...
        private void sendToLog(BufferedReader rdr) {
            ...
            if (lineCount > 0) {
                log.warn(sm.getString("cgiServlet.runStdErrCount", Integer
                        .valueOf(lineCount)));
            }
        }
    } //class CGIRunner
