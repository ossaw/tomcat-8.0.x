...
public abstract class RealmBase extends LifecycleMBeanBase implements Realm {
     ...
     *                                         not validated
     */
    public void setTransportGuaranteeRedirectStatus(
    ...
    public void setDigest(String digest) {
        ...
        if (ch instanceof MessageDigestCredentialHandler) {
            ...
            log.warn(sm.getString(
                    "realmBase.credentialHandler.customCredentialHandler",
                    "digest", digest));
        }
        this.digest = digest;
    }
     ...
     *
     * @param className The name of the class that will be used to extract user
     *                  names
     *                  from X509 client certificates.
    ...
    public Principal authenticate(String username, String clientDigest,
        ...
        String serverDigest = MD5Encoder.encode(ConcurrentMessageDigest
                .digestMD5(valueBytes));
    ...
    public Principal authenticate(X509Certificate certs[]) {
        ...
        if (validate) {
            for (int i = 0; i < certs.length; i++) {
                if (log.isDebugEnabled())
                    log.debug(" Checking validity for '" + certs[i]
    ...
    protected boolean compareCredentials(String userCredentials,
        ...
        if (hasMessageDigest()) {
            ...
            if (serverCredentials.startsWith("{MD5}") || serverCredentials
                ...
                // digested password.
                byte[] serverDigestPlusSaltBytes = Base64.decodeBase64(
                        serverDigestPlusSalt);
                ...
                // using the salt
                byte[] userDigestBytes = ConcurrentMessageDigest.digest(
                        getDigest(), userCredentials.getBytes(
                                StandardCharsets.ISO_8859_1), serverSaltBytes);
            ...
            for (int j = 0; j < collection.length; j++) {
                ...
                for (int k = 0; k < patterns.length; k++) {
                    ...
                    if (pattern.startsWith("/") && pattern.endsWith("/*")
                        ...
                        } else if (pattern.regionMatches(0, uri, 0, pattern
                                .length() - 1) || (pattern.length() - 2 == uri
                                        .length() && pattern.regionMatches(0,
                                                uri, 0, pattern.length()
                                                        - 2))) {
                            matched = true;
                            length = pattern.length();
                        }
    ...
    public boolean hasResourcePermission(Request request, Response response,
        ...
        if (!status) {
            response.sendError(HttpServletResponse.SC_FORBIDDEN, sm.getString(
                    "realmBase.forbidden"));
        }
        return status;
    ...
    public boolean hasUserDataPermission(Request request, Response response,
        ...
        if ((requestedSessionId != null) && request
    ...
    protected void initInternal() throws LifecycleException {
        ...
        x509UsernameRetriever = createUsernameRetriever(
    ...
    protected String digest(String credentials) {
        ...
        synchronized (this) {
            try {
                ...
                return (HexUtils.toHexString(ConcurrentMessageDigest.digest(
                        getDigest(), bytes)));
            } catch (Exception e) {
    ...
    public static void main(String args[]) {
        ...
        if (iterations > 0) {
            IntrospectionUtils.setProperty(handler, "iterations", Integer
                    .toString(iterations));
        }
        if (saltLength > -1) {
            IntrospectionUtils.setProperty(handler, "saltLength", Integer
                    .toString(saltLength));
        }
        if (keyLength > 0) {
            IntrospectionUtils.setProperty(handler, "keyLength", Integer
                    .toString(keyLength));
        }
