...
public class PoolableConnectionFactory implements
     ...
     * 
     * @param connectionInitSqls SQL statement to initialize
     *                           {@link Connection}s.
     */
    public void setConnectionInitSql(
     ...
     * 
     * @param defaultCatalog the default "catalog" setting for borrowed
     *                       {@link Connection}s
     */
     ...
     * </p>
     * <p>
     * If {@link #isFastFailValidation()} is {@code false} setting this property
     * has no
     * effect.
     * </p>
     *
    ...
    public PooledObject<PoolableConnection> makeObject() throws Exception {
        ...
        if (poolStatements) {
            ...
            if (dataSourceJmxName != null) {
                final StringBuilder base = new StringBuilder(dataSourceJmxName
                        .toString());
        ...
        final PoolableConnection pc = new PoolableConnection(conn, _pool,
                connJmxName, _disconnectionSqlCodes, _fastFailValidation);
        pc.setCacheState(_cacheState);

        return new DefaultPooledObject<>(pc);
    }

    protected void initializeConnection(final Connection conn)
            throws SQLException {
    ...
    public void validateConnection(final PoolableConnection conn)
            throws SQLException {
        if (conn.isClosed()) {
    ...
    public void activateObject(final PooledObject<PoolableConnection> p)
        ...
        if (_defaultAutoCommit != null && conn
                .getAutoCommit() != _defaultAutoCommit.booleanValue()) {
            conn.setAutoCommit(_defaultAutoCommit.booleanValue());
        }
        if (_defaultTransactionIsolation != UNKNOWN_TRANSACTIONISOLATION && conn
                .getTransactionIsolation() != _defaultTransactionIsolation) {
            conn.setTransactionIsolation(_defaultTransactionIsolation);
        }
        if (_defaultReadOnly != null && conn.isReadOnly() != _defaultReadOnly
                .booleanValue()) {
            conn.setReadOnly(_defaultReadOnly.booleanValue());
        }
        if (_defaultCatalog != null && !_defaultCatalog.equals(conn
                .getCatalog())) {
    ...
    private void validateLifetime(final PooledObject<PoolableConnection> p)
        ...
        if (maxConnLifetimeMillis > 0) {
            final long lifetime = System.currentTimeMillis() - p
                    .getCreateTime();
            if (lifetime > maxConnLifetimeMillis) {
                throw new LifetimeExceededException(Utils.getMessage(
                        "connectionFactory.lifetimeExceeded", Long.valueOf(
                                lifetime), Long.valueOf(
