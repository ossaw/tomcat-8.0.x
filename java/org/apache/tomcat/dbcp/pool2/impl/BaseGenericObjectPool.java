...
public abstract class BaseGenericObjectPool<T> extends BaseObject {
     ...
     * the factory associated with the pool. Returning objects that fail
     * validation
     ...
     *
     * @param testWhileIdle
     *                      <code>true</code> so objects will be validated by
     *                      the evictor
     *
     ...
     *                               max number of objects to examine during
     *                               each evictor run
     *
     * @see #getNumTestsPerEvictionRun
     * @see #setTimeBetweenEvictionRunsMillis
     */
    public final void setNumTestsPerEvictionRun(
     ...
     *                                   minimum amount of time an object may
     ...
     *                                       minimum amount of time an object
     *                                       may sit idle in the pool
     *                                       before it is eligible for eviction
     *                                       if minIdle instances are
     *                                       available
     *
    ...
    public final void setEvictionPolicyClassName(
        ...
        try {
            Class<?> clazz;
            try {
                clazz = Class.forName(evictionPolicyClassName, true, Thread
     ...
     * registered. Swallows MBeanRegistrationException,
    ...
    class Evictor extends TimerTask {
        ...
        @Override
        public void run() {
            final ClassLoader savedClassLoader = Thread.currentThread()
    ...
    private class StatsStore {
        ...
        public long getMean() {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (value != -1) {
                    counter++;
                    result = result * ((counter - 1) / (double) counter) + value
                            / (double) counter;
                }
            }
            return (long) result;
