 ...
 * - is the security enough to prevent Registry beeing used to avoid the
 * permission
...
public class Registry implements RegistryMBean, MBeanRegistration {
        ...
        if (server == null) {
            if (MBeanServerFactory.findMBeanServer(null).size() > 0) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Using existing MBeanServer " + (System
                            .currentTimeMillis() - t1));
                }
            } else {
                server = ManagementFactory.getPlatformMBeanServer();
                if (log.isDebugEnabled()) {
                    log.debug("Creating MBeanServer" + (System
                            .currentTimeMillis() - t1));
                }
            }
        }
    ...
    public Object convertValue(String type, String value) {
        ...
        if (type == null || "java.lang.String".equals(type)) {
            // string is default
            objValue = value;
        } else if ("javax.management.ObjectName".equals(type) || "ObjectName"
                .equals(type)) {
            try {
