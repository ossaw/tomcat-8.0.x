...
public final class LegacyCookieProcessor implements CookieProcessor {
    ...
    public LegacyCookieProcessor() {
         ...
         * FWD_SLASH_IS_SEPARATOR which, when false, means a '/' character will
         * not
    ...
    public void parseCookieHeader(MimeHeaders headers,
        ...
        while (pos >= 0) {
            ...
            if (cookieValue != null && !cookieValue.isNull()) {
                ...
                processCookieHeader(bc.getBytes(), bc.getOffset(), bc
    ...
    public String generateHeader(Cookie cookie) {
        ...
        if (version == 0) {
            ...
            if (needsQuotes(value, 0) || comment != null || needsQuotes(path, 0)
                    || needsQuotes(domain, 0)) {
                version = 1;
            }
        }
        ...
        if (maxAge >= 0) {
            ...
            if (version == 0 || getAlwaysAddExpires()) {
                ...
                if (maxAge == 0) {
                    ...
                    COOKIE_DATE_FORMAT.get().format(new Date(System
                            .currentTimeMillis() + maxAge * 1000L), buf,
                            new FieldPosition(0));
                }
            }
        }
    ...
    private static void escapeDoubleQuotes(StringBuffer b, String s,
    ...
    private boolean needsQuotes(String value, int version) {
        ...
        for (; i < len; i++) {
            ...
            if (version == 0 && !allowedWithoutQuotes.get(c) || version == 1
                    && isHttpSeparator(c)) {
                return true;
            }
        }
    ...
    private static boolean alreadyQuoted(String value) {
        return value.length() >= 2 && value.charAt(0) == '\"' && value.charAt(
                value.length() - 1) == '\"';
    }
    ...
    private final void processCookieHeader(byte bytes[], int off, int len,
        ...
        while (pos < end) {
            ...
            while (pos < end && (isHttpSeparator((char) bytes[pos])
                    && !getAllowHttpSepsInV0() || isV0Separator(
                            (char) bytes[pos]) || isWhiteSpace(bytes[pos]))) {
                pos++;
            }
            ...
            if (pos < (end - 1) && bytes[pos] == '=') {
                ...
                switch (bytes[pos]) {
                    ...
                    case ',':
                        ...
                        break;
                    default:
                        if (version == 0 && !isV0Separator((char) bytes[pos])
                                && getAllowHttpSepsInV0() || !isHttpSeparator(
                                        (char) bytes[pos])
                                || bytes[pos] == '=') {
                            // Token
                            valueStart = pos;
                            ...
                            // not valid.
                            UserDataHelper.Mode logMode = userDataLog
                                    .getNextMode();
                            if (logMode != null) {
            ...
            if (isSpecial) {
                ...
                if (equals("Domain", bytes, nameStart, nameEnd)) {
                    sc.getDomain().setBytes(bytes, valueStart, valueEnd
                            - valueStart);
                    continue;
                }

                if (equals("Path", bytes, nameStart, nameEnd)) {
                    sc.getPath().setBytes(bytes, valueStart, valueEnd
                            - valueStart);
                    continue;
                }
                ...
                if (valueStart != -1) { // Normal AVPair
                    sc.getValue().setBytes(bytes, valueStart, valueEnd
                            - valueStart);
                    if (isQuoted) {
    ...
    private final int getTokenEndPosition(byte bytes[], int off, int end,
        ...
        int pos = off;
        while (pos < end && (!isHttpSeparator((char) bytes[pos]) || version == 0
                && getAllowHttpSepsInV0() && bytes[pos] != '='
                && !isV0Separator((char) bytes[pos]) || !isName
                        && bytes[pos] == '=' && getAllowEqualsInValue())) {
            pos++;
        }
    ...
    private static final boolean equals(String s, byte b[], int start,
            int end) {
        int blen = end - start;
