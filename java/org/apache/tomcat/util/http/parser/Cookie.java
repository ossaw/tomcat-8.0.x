...
public class Cookie {
    ...
    private static final UserDataHelper invalidCookieVersionLog = new UserDataHelper(
            log);
    private static final UserDataHelper invalidCookieLog = new UserDataHelper(
            log);
    ...
    private static final byte[] PATH_BYTES = "$Path".getBytes(
            StandardCharsets.ISO_8859_1);
    private static final byte[] DOMAIN_BYTES = "$Domain".getBytes(
            StandardCharsets.ISO_8859_1);
    ...
    static {
        ...
        for (int i = 0; i < 256; i++) {
            if (i < TAB_BYTE || (i > TAB_BYTE && i < SPACE_BYTE)
                    || i == DEL_BYTE) {
                isText[i] = false;
            } else {
                isText[i] = true;
    ...
    private static void parseCookieRfc6265(ByteBuffer bb,
            ServerCookies serverCookies) {
        ...
        while (moreToProcess) {
            ...
            if (name.hasRemaining()) {
                ...
                sc.getName().setBytes(name.array(), name.position(), name
                        .remaining());
                if (value == null) {
    ...
    private static void parseCookieRfc2109(ByteBuffer bb,
            ServerCookies serverCookies) {
        ...
        while (moreToProcess) {
            ...
            if (name.hasRemaining() && value != null && value.hasRemaining()) {
                ...
                sc.getName().setBytes(name.array(), name.position(), name
                        .remaining());
                sc.getValue().setBytes(value.array(), value.position(), value
                        .remaining());
                if (domain != null) {
                    sc.getDomain().setBytes(domain.array(), domain.position(),
                            domain.remaining());
                }
                if (path != null) {
                    sc.getPath().setBytes(path.array(), path.position(), path
                            .remaining());
                }
            }
        }
    ...
    private static ByteBuffer readCookieValue(ByteBuffer bb) {
        ...
        while (bb.hasRemaining()) {
            ...
            if (isCookieOctet[(b & 0xFF)]) {
                // NO-OP
            } else if (b == SEMICOLON_BYTE || b == COMMA_BYTE || b == SPACE_BYTE
                    || b == TAB_BYTE) {
                end = bb.position() - 1;
                bb.position(end);
                break;
    ...
    private static ByteBuffer readCookieValueRfc6265(ByteBuffer bb) {
        ...
        while (bb.hasRemaining()) {
            ...
            if (isCookieOctet[(b & 0xFF)]) {
                // NO-OP
            } else if (b == SEMICOLON_BYTE || b == SPACE_BYTE
                    || b == TAB_BYTE) {
                end = bb.position() - 1;
                bb.position(end);
                break;
    ...
    private static ByteBuffer readCookieValueRfc2109(ByteBuffer bb,
    ...
    private static void logInvalidHeader(ByteBuffer bb) {
        ...
        if (logMode != null) {
            String headerValue = new String(bb.array(), bb.position(), bb
                    .limit() - bb.position(), StandardCharsets.UTF_8);
    ...
    private static void logInvalidVersion(ByteBuffer value) {
        ...
        if (logMode != null) {
            ...
            if (value == null) {
                ...
                version = new String(value.bytes, value.position(), value
                        .limit() - value.position(), StandardCharsets.UTF_8);
            }
