...
public class NioSelectorPool {
    ...
    protected static final boolean SHARED = Boolean.parseBoolean(System
            .getProperty("org.apache.tomcat.util.net.NioSelectorShared",
                    "true"));
    ...
    public void put(Selector s) throws IOException {
        ...
        if (enabled)
            active.decrementAndGet();
        if (enabled && (maxSpareSelectors == -1 || spare.get() < Math.min(
     ...
     * 
     * @param buf          The buffer containing the data, we will write as long
     *                     as <code>(buf.hasRemaining()==true)</code>
     * @param socket       The socket to write data to
     * @param selector     The selector to use for blocking, if null then a busy
     *                     write will be initiated
     * @param writeTimeout The timeout for this write operation in milliseconds,
     *                     -1 means no timeout
     ...
     * @throws IOException            if an IO Exception occurs in the
     *                                underlying socket logic
     */
        ...
        try {
            while ((!timedout) && buf.hasRemaining()) {
                ...
                if (writeTimeout > 0 && (selector == null || keycount == 0))
                    timedout = (System.currentTimeMillis()
                            - time) >= writeTimeout;
            } //while
            if (timedout)
     ...
     *                    read as until we have read at least one byte or we
     *                    timed out
     ...
     * @param readTimeout long - the timeout for this read operation in
     *                    milliseconds, -1 means no timeout
     * @param block       - true if you want to block until data becomes
     *                    available or timeout time has been reached
     * @return int - returns the number of bytes read
     * @throws EOFException           if read returns -1
     * @throws SocketTimeoutException if the read times out
     * @throws IOException            if an IO Exception occurs in the
