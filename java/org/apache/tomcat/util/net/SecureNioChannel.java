...
public class SecureNioChannel extends NioChannel {
     ...
     *                                application or network buffers contain
     *                                data
    ...
    public void rehandshake(long timeout) throws IOException {
        ...
        if (getBufHandler().getReadBuffer().position() > 0 && getBufHandler()
                .getReadBuffer().position() < getBufHandler().getReadBuffer()
                        .limit())
            throw new IOException(sm.getString(
                    "channel.nio.ssl.appInputNotEmpty"));
        if (getBufHandler().getWriteBuffer().position() > 0 && getBufHandler()
                .getWriteBuffer().position() < getBufHandler().getWriteBuffer()
                        .limit())
            throw new IOException(sm.getString(
            ...
            while (handshaking) {
                ...
                switch (hsStatus) {
                    ...
                    default: {
                        ...
                        if (keyCount == 0 && ((System.currentTimeMillis()
                                - now) >= timeout)) {
                            throw new SocketTimeoutException(sm.getString(
                                    "channel.nio.ssl.timeoutDuringHandshake"));
                        }
    ...
    public int read(ByteBuffer dst) throws IOException {
        ...
        do {
            ...
            if (unwrap.getStatus() == Status.OK || unwrap
                ...
                //in the constructor
                throw new IOException(sm.getString("channel.nio.ssl.unwrapFail",
                        unwrap.getStatus()));
            }
