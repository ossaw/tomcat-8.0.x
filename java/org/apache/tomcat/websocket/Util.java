...
public class Util {
    ...
    private static <T> TypeResult getGenericType(Class<T> type,
            ...
            if (iface instanceof ParameterizedType) {
                ...
                if (pi.getRawType() instanceof Class) {
                    if (type.isAssignableFrom((Class<?>) pi.getRawType())) {
                        return getTypeParameter(clazz, pi
        ...
        if (superClassTypeResult.getDimension() > 0) {
            ...
            className.append('L');
            className.append(superClassTypeResult.getClazz()
                    .getCanonicalName());
            className.append(';');
    ...
    private static TypeResult getTypeParameter(Class<?> clazz, Type argType) {
        if (argType instanceof Class<?>) {
            ...
            return new TypeResult((Class<?>) ((ParameterizedType) argType)
                    .getRawType(), -1, 0);
            ...
            for (Class<? extends Decoder> decoderClazz : decoderClazzes) {
                ...
                try {
                    ...
                    throw new DeploymentException(sm.getString(
                            "pojoMethodMapping.invalidDecoder", decoderClazz
                                    .getName()), e);
                }
                DecoderEntry entry = new DecoderEntry(Util.getDecoderType(
                        decoderClazz), decoderClazz);
                result.add(entry);
            }
        }
    ...
    static Set<MessageHandlerResult> getMessageHandlers(Class<?> target,
        ...
        if (String.class.isAssignableFrom(target)) {
            ...
            boolean whole = MessageHandler.Whole.class.isAssignableFrom(listener
                    .getClass());
            MessageHandlerResult result = new MessageHandlerResult(whole
                    ? new PojoMessageHandlerWholeBinary(listener,
                            ...
                            endpointConfig, matchDecoders(target,
                                    endpointConfig, true), new Object[1], 0,
                            true, -1, false, -1)
            ...
            MessageHandlerResult result = new MessageHandlerResult(
                    new PojoMessageHandlerWholeText(listener,
                            getOnMessageMethod(listener), session,
                            endpointConfig, matchDecoders(target,
                                    endpointConfig, false), new Object[1], 0,
                            true, -1, -1), MessageHandlerResultType.TEXT);
            results.add(result);
        } else {
            ...
            if (decoderMatch.getBinaryDecoders().size() > 0) {
                MessageHandlerResult result = new MessageHandlerResult(
                        new PojoMessageHandlerWholeBinary(listener, m, session,
                                endpointConfig, decoderMatch
                                        .getBinaryDecoders(), new Object[1], 0,
                                false, -1, false, -1),
                        MessageHandlerResultType.BINARY);
                results.add(result);
            }
    ...
    public static void parseExtensionHeader(List<Extension> extensions,
        ...
        for (String unparsedExtension : unparsedExtensions) {
            ...
            WsExtension extension = new WsExtension(unparsedParameters[0]
                    .trim());
            ...
            for (int i = 1; i < unparsedParameters.length; i++) {
                ...
                if (equalsPos == -1) {
                    ...
                    value = unparsedParameters[i].substring(equalsPos + 1)
                            .trim();
    ...
    public static class DecoderMatch {
        ...
        public DecoderMatch(Class<?> target,
