...
public class PojoMethodMapping {
            ...
            for (Method method : currentClazzMethods) {
                if (method.getAnnotation(OnOpen.class) != null) {
                    ...
                    if (open == null) {
                        ...
                        if (currentClazz == clazzPojo || !isMethodOverride(open,
                                method)) {
                    ...
                    if (close == null) {
                        ...
                        if (currentClazz == clazzPojo || !isMethodOverride(
                                close, method)) {
                            // Duplicate annotation
                            throw new DeploymentException(sm.getString(
                                    "pojoMethodMapping.duplicateAnnotation",
                    ...
                    checkPublic(method);
                    MessageHandlerInfo messageHandler = new MessageHandlerInfo(
                            method, decoders);
                    boolean found = false;
                    for (MessageHandlerInfo otherMessageHandler : onMessage) {
                        if (messageHandler.targetsSameWebSocketMessageType(
                                otherMessageHandler)) {
                            found = true;
                            if (currentClazz == clazzPojo || !isMethodOverride(
                                    messageHandler.m, otherMessageHandler.m)) {
                                // Duplicate annotation
                                throw new DeploymentException(sm.getString(
                                        "pojoMethodMapping.duplicateAnnotation",
    ...
    private boolean isMethodOverride(Method method1, Method method2) {
        return (method1.getName().equals(method2.getName()) && method1
                .getReturnType().equals(method2.getReturnType()) && Arrays
                        .equals(method1.getParameterTypes(), method2
                                .getParameterTypes()));
    }
    ...
    private boolean isOverridenWithoutAnnotation(Method[] methods,
        ...
        for (Method method : methods) {
            if (isMethodOverride(method, superclazzMethod) && (method
    ...
    private static PojoPathParam[] getPathParams(Method m,
        ...
        for (int i = 0; i < types.length; i++) {
            ...
            if (type.equals(Session.class)) {
            ...
            } else if (methodType == MethodType.ON_OPEN && type.equals(
                    EndpointConfig.class)) {
                result[i] = new PojoPathParam(type, null);
            } else if (methodType == MethodType.ON_ERROR && type.equals(
                    Throwable.class)) {
                foundThrowable = true;
                result[i] = new PojoPathParam(type, null);
            } else if (methodType == MethodType.ON_CLOSE && type.equals(
                    CloseReason.class)) {
                result[i] = new PojoPathParam(type, null);
            } else {
                ...
                if (result[i] == null) {
                    throw new DeploymentException(sm.getString(
                            "pojoMethodMapping.paramWithoutAnnotation", type, m
                                    .getName(), m.getClass().getName()));
                }
            }
        }
    ...
    private static class MessageHandlerInfo {
                ...
                if (String.class.isAssignableFrom(types[i])) {
                    if (indexString == -1) {
                        ...
                        throw new IllegalArgumentException(sm.getString(
                                "pojoMethodMapping.duplicateMessageParam", m
                    ...
                    if (indexBoolean == -1) {
                        ...
                        throw new IllegalArgumentException(sm.getString(
                                "pojoMethodMapping.duplicateLastParam", m
                                        .getName(), m.getDeclaringClass()
                                                .getName()));
                    }
                    ...
                    if (indexSession == -1) {
                        ...
                        throw new IllegalArgumentException(sm.getString(
                                "pojoMethodMapping.duplicateSessionParam", m
                                        .getName(), m.getDeclaringClass()
                                                .getName()));
                    }
                    ...
                    if (indexPong == -1) {
                        ...
                        throw new IllegalArgumentException(sm.getString(
                                "pojoMethodMapping.duplicatePongMessageParam", m
                                        .getName(), m.getDeclaringClass()
                                                .getName()));
                    }
                } else {
        ...
        public boolean targetsSameWebSocketMessageType(
                MessageHandlerInfo otherHandler) {
            ...
            if (decoderMatch != null && otherHandler.decoderMatch != null
                    && decoderMatch.getTarget().equals(otherHandler.decoderMatch
                            .getTarget())) {
                return true;
            }
            return false;
        ...
        public Set<MessageHandler> getMessageHandlers(Object pojo,
            ...
            for (Map.Entry<Integer, PojoPathParam> entry : indexPathParams
                ...
                try {
                    ...
                    DecodeException de = new DecodeException(valueString, sm
                            .getString("pojoMethodMapping.decodePathParamFail",
                ...
                if (indexString != -1 || indexPrimitive != -1) {
                    ...
                    if (decoderMatch.getBinaryDecoders().size() > 0) {
                        MessageHandler mh = new PojoMessageHandlerWholeBinary(
                                pojo, m, session, config, decoderMatch
