...
class JspReader {
    ...
    public JspReader(JspCompilationContext ctxt, String fname, String encoding,
            Jar jar, ErrorDispatcher err) throws JasperException,
     ...
     */
    public JspReader(JspCompilationContext ctxt, String fname,
    ...
    Mark skipUntilIgnoreEsc(String limit, boolean ignoreEL) {
        ...
        char firstChar = limit.charAt(0);
        skip: for (ch = nextChar(ret); ch != -1; prev = ch, ch = nextChar(
                ret)) {
            if (ch == '\\' && prev == '\\') {
                ch = 0; // Double \ is not an escape char anymore
            } else if (prev == '\\') {
                continue;
            } else if (!ignoreEL && (ch == '$' || ch == '#')
                    && peekChar() == '{') {
                // Move beyond the '{'
                nextChar();
                skipELExpression();
    ...
    String parseToken(boolean quoted) throws JasperException {
        ...
        if (quoted) {
            if (ch == '"' || ch == '\'') {
                ...
                ch = nextChar();
                for (ch = nextChar(); ch != -1
                        && ch != endQuote; ch = nextChar()) {
                    if (ch == '\\')
                        ch = nextChar();
